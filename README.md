...
> **Build natif avec la toolchain GNU (sans MSVC).** L'installeur `0.3.1` de [dist/](dist/) a été produit ainsi, sans les *MS C++ Build Tools* : `rustup default stable-x86_64-pc-windows-gnu` + **mingw-w64 sur le `PATH`** (fournit `dlltool` et `windres`, que la toolchain Rust GNU n'embarque pas). Deux pièges rencontrés :
