 ...
 */
export function emojiTokenAt(text: string, caret: number): { start: number; query: string } | null {
 ...
 */
export function queryEmojiShortcodes(
  query: string | undefined | null,
  limit = 8
): { name: string; char: string }[] {
  const q = String(query || "").toLowerCase();
  if (!q) return [];
  const starts: { name: string; char: string }[] = [];
  const contains: { name: string; char: string }[] = [];
