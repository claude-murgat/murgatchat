...
import { uploadFile, api } from "../api.ts";
import { emojiTokenAt, queryEmojiShortcodes } from "../emojiShortcodes.ts";
import type { Attachment, User } from "../types.ts";

/** Jeton de mention « @… » en cours de saisie, repéré autour du curseur. */
interface MentionQuery {
  start: number;
  query: string;
}

/** Suggestion d'emoji telle que renvoyée par `queryEmojiShortcodes`. */
interface EmojiSuggestion {
  name: string;
  char: string;
}

/** GIF choisi dans le GifPicker (seule l'URL pleine taille est consommée ici). */
interface GifChoice {
  fullUrl: string;
}

/** Brouillon d'une conversation, conservé par le parent (#165). */
interface Draft {
  text: string;
  attachments: Attachment[];
}

/** Charge utile transmise à `onSend` (`scheduledAt` seulement si planifié). */
...
/**
 * Erreur levée par `api`/`uploadFile` : le corps JSON de la réponse est attaché
 * sur `data` (cf. `Object.assign(new Error(...), { data, status })` dans api.ts).
 */
interface ApiError extends Error {
  data?: { error?: string; maxMb?: number };
  status?: number;
}

/** Poignée exposée au parent via la ref (upload des fichiers déposés). */
...
function formatLocalIso(dt: Date) {
...
function renderComposerText(value: string) {
...
function Composer(
  ...
  const [emojiItems, setEmojiItems] = useState<EmojiSuggestion[]>([]);
  ...
  useEffect(() => {
    if (!showGif) return;
    function onDocMouseDown(e: MouseEvent) {
      if (gifRef.current?.contains(e.target as Node)) return;
      if (gifBtnRef.current?.contains(e.target as Node)) return;
      setShowGif(false);
    }
    document.addEventListener("mousedown", onDocMouseDown);
    return () => document.removeEventListener("mousedown", onDocMouseDown);
  ...
  async function onGifSelect(gif: GifChoice) {
    setShowGif(false);
    setGifBusy(true);
    try {
      ...
      alert((err as ApiError | undefined)?.message || "Échec de l'envoi du GIF");
    } finally {
      setGifBusy(false);
    }
  }
  ...
  async function ingestFiles(files: File[]) {
    ...
    try {
      ...
      if ((err as ApiError | undefined)?.data?.error === "file_too_large") {
        const mb = (err as ApiError | undefined)?.data?.maxMb;
        alert(mb ? `Fichier trop volumineux (max ${mb} Mo).` : "Fichier trop volumineux.");
      } else {
        alert((err as ApiError).message || "Upload échoué");
      }
    } finally {
      setUploading(false);
    }
  ...
  async function onPaste(e: ClipboardEvent<HTMLTextAreaElement>) {
    ...
    // `filter(Boolean)` ne restreint pas le type : les `null` de getAsFile() sont
    // bien écartés au runtime, on le déclare au compilateur.
    const files = items
  ...
  function refreshEmojiMenu(value: string, caret: number) {
  ...
  function send(schedule = false) {
    ...
    const payload: SendPayload = { body, attachmentIds: attachments.map((a) => a.id) };
    if (schedule) {
  ...
  function applyMention(member: User | undefined) {
