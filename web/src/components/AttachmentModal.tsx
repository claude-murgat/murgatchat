...
/** Valeur d'une cellule de tableur : typée par read-excel-file, texte en CSV. */
type Cell = string | number | boolean | Date | null | undefined;
/** Ligne d'une grille de tableur. */
type Row = Cell[];

/** Stratégie d'aperçu retenue pour une pièce jointe (voir `kindOf`). */
type PreviewKind =
  | "image"
  | "video"
  | "audio"
...
function kindOf(mime = "", filename = ""): PreviewKind {
...
function FallbackCard({ attachment, onDownload, note }: FallbackCardProps) {
  return (
...
function useFetchedBuffer(url: string) {
  const [state, setState] = useState<BufferState>({ status: "loading" });
  useEffect(() => {
    let alive = true;
...
function SheetGrid({ rows }: SheetGridProps) {
...
// Les typages publiés de read-excel-file ne décrivent qu'une seule signature
// (`Promise<Sheet[]>`) et ignorent les options `getSheets` / `sheet` que la v9
// accepte réellement : on redéclare ici la forme large effectivement appelée, et
// on affine le résultat au cas par cas (voir la normalisation ci-dessous).
type XlsxReader = (
  input: Blob,
  options?: { getSheets?: boolean; sheet?: number | string },
) => Promise<unknown>;

/** Entrée brute de la liste de feuilles, v9 (`{ sheet, data }`) ou ancienne (`{ name }`). */
interface RawSheet {
  name?: string;
  sheet?: string;
  data?: unknown;
...
/** Feuille normalisée : un nom d'onglet et, si déjà parsées, ses lignes. */
...
function XlsxView({ buffer, onError }: DocumentViewProps) {
  const [sheets, setSheets] = useState<SheetEntry[] | null>(null);
  const [active, setActive] = useState(0);
  const [rows, setRows] = useState<Row[] | null>(null);
  const [truncated, setTruncated] = useState(false);
  const readRef = useRef<XlsxReader | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        readRef.current = mod.default as unknown as XlsxReader;
        blobRef.current = new Blob([buffer]);
        const list = (await readRef.current(blobRef.current, { getSheets: true })) as RawSheet[];
        ...
        const norm: SheetEntry[] = (
          Array.isArray(list) && list.length ? list : [{} as RawSheet]
        ).map((s, i) => ({
          name: s.name ?? s.sheet ?? `Feuille ${i + 1}`,
          data: Array.isArray(s.data) ? (s.data as Row[]) : null,
        }));
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!data)
          data = (await readRef.current!(blobRef.current!, { sheet: active + 1 })) as Row[];
        let r: Row[] = Array.isArray(data) ? data : [];
...
function CsvView({ buffer, onError }: DocumentViewProps) {
  const rows = useMemo(() => {
    try {
...
function DocxView({ buffer, onError }: DocumentViewProps) {
...
function TextView({ buffer, onError }: DocumentViewProps) {
  const text = useMemo(() => {
    try {
...
function PdfView({ buffer, attachment }: PdfViewProps) {
...
function DocumentPreview({ url, kind, attachment, onDownload }: DocumentPreviewProps) {
