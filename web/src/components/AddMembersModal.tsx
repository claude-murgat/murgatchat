...
interface AddMembersModalProps {
  ...
  onClose: () => void;
  /** Reçoit le salon re-sérialisé par le serveur après l'ajout. */
...
export default function AddMembersModal({
  ...
  const memberIds = useMemo(
    () => new Set((channel.members || []).map((m: User) => m.id)),
    [channel.members]
  );
  ...
  useEffect(() => {
    ...
    api.listUsers(q).then((res) => {
      if (!cancelled) {
        setUsers(
          res.users.filter((u: User) => u.id !== currentUserId && !memberIds.has(u.id))
        );
      }
    });
    return () => {
