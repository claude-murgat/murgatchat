...
interface UpdateBannerProps {
  ...
  onDismiss: () => void;
  /** Absent hors desktop (ou sans updater Tauri câblé) : on retombe alors sur
   *  l'ouverture de la page de téléchargement. */
  onDesktopInstall?: (() => Promise<void>) | null;
}

export default function UpdateBanner({ info, onDismiss, onDesktopInstall }: UpdateBannerProps) {
  ...
  async function action() {
    if (canAutoUpdate) {
      ...
      // Assertions : le garde-fou `if (!info?.updateAvailable) return null` en
      // tête de composant a déjà écarté `info` nul et la charge /version fournit
      // l'URL avec la mise à jour — mais l'analyse de flux ne traverse pas cette
      // fonction imbriquée.
