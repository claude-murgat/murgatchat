...
/** Un tour de la conversation de support, tel que renvoyé par l'API. */
...
/**
 * Erreur levée par `api.ts` : une `Error` à laquelle le corps JSON de la
 * réponse a été greffé (`Object.assign(new Error(...), { data })`).
 */
...
export default function BugReportModal({ onClose }: BugReportModalProps) {
  ...
  const [files, setFiles] = useState<File[]>([]); // pending screenshots, uploaded on send
  ...
  const [thread, setThread] = useState<SupportMessage[]>([]); // [{ role, content }]
  ...
  async function uploadAttachments() {
    ...
    // `uploadFile` est un export AUTONOME de api.ts, pas un membre de l'objet
    // `api` : `api.uploadFile` valait `undefined` et joindre une capture d'écran
    // à un signalement levait « api.uploadFile is not a function ». Bug révélé
    // par la migration TypeScript. Composer.tsx faisait déjà l'appel correct.
    const uploaded = await Promise.all(files.map((f) => uploadFile(f)));
    return uploaded.map((a: { id: string }) => a.id);
  }

  // Last resort when the chat is unavailable: file a plain one-shot report.
  async function oneShotFallback(text: string, attachmentIds: string[] = []) {
    await api.reportBug({
      message: text,
  ...
  async function start() {
    ...
    try {
      ...
      const code = (e as ApiError | undefined)?.data?.error;
      if (code === "support_chat_unavailable" || code === "support_chat_error") {
        try {
          ...
          setError(
            (e2 as ApiError | undefined)?.data?.error ||
              (e2 as ApiError | undefined)?.message ||
        ...
        setError(
          (e as ApiError | undefined)?.data?.error ||
            (e as ApiError | undefined)?.message ||
            "Échec de l'envoi — réessayez."
        );
        setPhase("compose");
      }
  ...
  async function send() {
    ...
    setInput("");
    try {
      // `send()` n'est atteignable qu'en phase « chat », où la conversation a
      // forcément été créée ; TS ne peut pas le déduire du flux de composant.
