...
interface DndModalProps {
  /** Vrai quand un « ne pas déranger » est déjà en cours. */
  active: boolean;
  user: User | null;
  onClose: () => void;
  /** Minutes de silence demandées (0 = désactiver tout de suite). */
  onPick: (minutes: number) => void;
  onSaveSchedule: (schedule: DndSchedule) => Promise<void> | void;
}
...
function minutesUntilTomorrow(hour: number) {
  const now = new Date();
