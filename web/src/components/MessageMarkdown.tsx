...
// Vue minimale d'un nœud mdast : c'est tout ce que le parcours ci-dessous lit
// ou produit. Le nœud « mention » fabriqué plus bas n'est pas un nœud mdast
// standard (il ne vit que par son `data.hName`), d'où cette forme souple
// plutôt que les types stricts de @types/mdast.
...
export function remarkMentions() {
  return (tree: MdastNodeLike) => transformMentions(tree);
}

function transformMentions(node: MdastNodeLike) {
  if (!Array.isArray(node.children)) return;
  const next: MdastNodeLike[] = [];
  for (const child of node.children) {
    if (child.type === "text") {
      // Un nœud `text` porte toujours un `value` (garanti par le parseur mdast).
      next.push(...splitMentions(child.value as string));
    } else {
...
function splitMentions(value: string): MdastNodeLike[] {
  const out: MdastNodeLike[] = [];
  let last = 0;
  let m;
...
function mentionNode(name: string): MdastNodeLike {
  return {
    type: "mention",
    data: {
      hName: "span",
...
// `inline` n'est plus déclaré dans les props de react-markdown v9+, mais reste
// la façon de distinguer un `code` inline du contenu d'un bloc `pre` : on le
// réintroduit côté type, en optionnel, pour garder le rendu tel quel.
