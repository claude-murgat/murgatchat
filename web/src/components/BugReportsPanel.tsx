...
/** État d'un rapport, tel que validé par PATCH /bug-reports/:id. */
type BugReportStatus = "open" | "closed";

/** Auteur d'un rapport (sous-ensemble de User exposé par la route). */
interface BugReportAuthor {
  id: string;
  username: string;
  displayName: string;
  avatarColor: string;
...
/** Rapport de bug tel que sérialisé par server/src/routes/bugReports.ts. */
interface BugReport {
  id: string;
  message: string;
  /** Payload libre envoyé par le client (z.any() côté serveur). */
  diagnostics: unknown;
  logs: string | null;
  ...
  createdAt: string;
  user: BugReportAuthor | null;
}

/** Erreur levée par `request()` : la charge JSON du serveur y est attachée. */
type ApiError = Error & { data?: { error?: string } };
      ...
      {Array.isArray(report.attachments) && report.attachments.length > 0 && (
        <div>
          ...
          <div className="flex flex-wrap gap-2">
            {report.attachments.map((a: Attachment) => (
              <a
                key={a.id}
...
function FilterButton({ value, label, active, onSelect }: FilterButtonProps) {
  return (
    <button
...
/** Arguments du chargement paginé (`append` = concaténer au lieu de remplacer). */
...
export default function BugReportsPanel() {
  ...
  async function fetchPage({ p, status, append }: FetchPageArgs) {
  ...
  async function changeStatus(report: BugReport, status: BugReportStatus) {
    ...
    try {
      ...
      setError((e as ApiError).data?.error || (e as ApiError).message || "Erreur");
    } finally {
      setBusyId(null);
    }
  }
