...
/** Un GIF normalisé par le proxy GIPHY du serveur (GET /gifs/search, mapGiphy). */
...
export default function GifPicker({ onSelect }: GifPickerProps) {
  ...
  async function load(query: string, pos: number, append: boolean) {
    const s = ++seq.current;
    ...
    try {
      ...
      if (s !== seq.current) return;
      // `request()` (api.ts) attache le corps JSON de la réponse à l'erreur.
      if ((e as { data?: { error?: string } } | undefined)?.data?.error === "not_configured") {
