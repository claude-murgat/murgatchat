...
// Formes DÉRIVÉES des contrats partagés et des props du Composer : rien n'est
// redéclaré ici, donc rien ne peut diverger de la source de vérité.
type Reaction = Message["reactions"][number];
type ReactionUser = Reaction["users"][number];
type ComposerProps = React.ComponentProps<typeof Composer>;
type Draft = NonNullable<ComposerProps["initialDraft"]>;
type SendPayload = Parameters<ComposerProps["onSend"]>[0];

/** Message planifié (GET /channels/:id/scheduled) : `scheduledAt` y est toujours renseigné. */
type ScheduledMessage = Pick<Message, "id" | "body"> & {
  // Date côté serveur, chaîne ISO une fois passée sur le fil.
  scheduledAt: string | Date;
};

/** Champs modifiables d'un message planifié (PATCH /channels/scheduled/:id). */
interface ScheduledPatch {
  body?: string;
  scheduledAt?: string;
}

/** Accusé de réception du serveur sur "message:send". */
interface SendAck {
  error?: string;
  scheduled?: ScheduledMessage;
}

/** Intention de scroll appliquée après une maj de `messages` (cf. le useLayoutEffect). */
type ScrollMode =
  | { type: "bottom" }
  | { type: "unread" }
...
function typingLabel(userIds: string[], channel: Channel, currentUser: User | null) {
  const names = userIds
...
function reactionLabel(users: ReactionUser[]) {
...
function previewSnippet(body: string | null | undefined, max = 60) {
...
function buildForwardBody(message: Message) {
...
export default function ChannelView({
  ...
  const scrollModeRef = useRef<ScrollMode | null>({ type: "bottom" });
  ...
  useEffect(() => {
    ...
    function onDocMouseDown(e: MouseEvent) {
      if (!notifyMenuRef.current?.contains(e.target as Node)) setShowNotifyMenu(false);
    }
  ...
  async function changeNotifyLevel(level: NotifyLevel) {
    setShowNotifyMenu(false);
  ...
  function send(payload: SendPayload) {
    ...
    socket.emit(
      ...
      (resp?: SendAck) => {
        ...
        if (resp?.scheduled) {
          // Le raffinement de `resp.scheduled` (test juste au-dessus) ne survit pas
          // au passage dans l'updater : l'assertion le rétablit, sans effet runtime.
  ...
  function forwardMessage(targetChannel: Channel) {
  ...
  async function updateScheduled(id: string, body: ScheduledPatch) {
    try {
      const res: { scheduled: ScheduledMessage } = await api.updateScheduled(id, body);
      ...
      >
        ...
        {messages.map((m, idx) => {
          ...
          const groupWithPrev =
            ...
            // Soustraction de deux Date : JS les convertit en millisecondes ; le
            // cast ne fait que l'expliciter pour TS (rien ne change à l'exécution).
            (new Date(m.createdAt) as unknown as number) -
              (new Date(prev.createdAt) as unknown as number) <
              5 * 60_000 &&
            !showDay;
          return (
            <div key={m.id}>
...
function toLocalIso(value: string | Date) {
...
interface ScheduledRowProps {
  ...
  onCancel: () => void;
  onSave: (payload: ScheduledPatch) => Promise<boolean>;
}

function ScheduledRow({ message, onCancel, onSave }: ScheduledRowProps) {
...
function QuoteBubble({ parent, onJump }: QuoteBubbleProps) {
...
function MessageRow({
  ...
  useEffect(() => {
    ...
    function onDocMouseDown(e: MouseEvent) {
      ...
      if (reactBtnRef.current?.contains(e.target as Node)) return;
      setShowPicker(false);
    }
    document.addEventListener("mousedown", onDocMouseDown);
    return () => document.removeEventListener("mousedown", onDocMouseDown);
