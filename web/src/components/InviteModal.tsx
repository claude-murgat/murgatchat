...
interface Invitation {
  ...
  /** Ni acceptée ni expirée. */
  pending: boolean;
  link: string;
}

/** Retour de POST /auth/invitations, réduit à ce qu'affiche l'écran. */
...
export default function InviteModal({ onClose }: InviteModalProps) {
  ...
  const [result, setResult] = useState<InviteResult | null>(null); // { link, emailSent, token }
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      // `request()` (api.ts) ne rejette qu'avec des Error : simple vue typée.
