// Les types Tauri sont importés en `import type` : purement compilatoire, aucun
// import émis — les modules restent chargés dynamiquement (build navigateur intact).
import type { Window as TauriWindow } from "@tauri-apps/api/window";
import type { Update } from "@tauri-apps/plugin-updater";

let mode: "tauri" | "browser" | null = null;
...
async function refreshDesktopState(win: TauriWindow, focusedHint?: boolean) {
  try {
...
async function initDesktopPresence() {
  try {
    ...
    console.warn("[desktop] presence tracking init failed:", (e as Error)?.message || e);
  }
}
...
async function initNotificationClicks() {
  try {
    ...
    await listen<string | null>("desktop:notification-click", (event) => {
      window.dispatchEvent(
    ...
    console.warn("[desktop] notification-click listener init failed:", (e as Error)?.message || e);
  }
}
    ...
    try {
      ...
      if (reg && typeof reg.showNotification === "function") {
        await reg.showNotification(title, {
          ...
          // `renotify` existe dans la spec Notifications mais pas encore dans les
          // typages DOM de TypeScript, d'où l'assertion (aucun effet à l'exécution).
        } as NotificationOptions & { renotify?: boolean });
        return;
      }
    } catch (e) {
      console.warn("[desktop] SW notification failed, fallback in-page:", (e as Error)?.message || e);
    }
...
export function setTrayBadge(unread: boolean) {
  if (!isTauri()) return;
...
export async function installDesktopUpdate(update: Update) {
