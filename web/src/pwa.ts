...
import { api, getApiBaseUrl, getToken } from "./api.ts";
...
let swRegistration: ServiceWorkerRegistration | null | undefined = null;
let lastVapidKey: string | null = null;
...
async function teardownServiceWorker() {
  try {
    ...
    console.warn("[pwa] desktop SW teardown failed:", (e as Error)?.message || e);
  }
}
...
export async function ensurePwaReady() {
  ...
  try {
    ...
    console.warn("[pwa] init failed:", (e as Error)?.message || e);
    return { supported: true, error: (e as Error)?.message || String(e) };
  }
}
...
export async function resubscribeIfNeeded() {
  ...
  try {
    ...
    console.warn("[pwa] resubscribe failed:", (e as Error)?.message || e);
  }
}
...
export async function unsubscribePush() {
  try {
    ...
    console.warn("[pwa] unsubscribe failed:", (e as Error)?.message || e);
  }
}
...
async function ensureSubscribed() {
  ...
  if (!sub) {
    ...
    try {
      ...
      console.warn("[pwa] subscribe failed:", (e as Error)?.message || e);
      return null;
    }
  }
...
async function fetchVapidKey() {
  ...
  try {
    ...
    console.warn("[pwa] vapid fetch failed:", (e as Error)?.message || e);
    return null;
  }
}
...
async function syncSubscriptionToBackend(
  ...
  const keys: Record<string, string> = raw?.keys || {};
  const endpoint = sub.endpoint;
  ...
  try {
    ...
    console.warn("[pwa] backend subscribe sync failed:", (e as Error)?.message || e);
  }
}

/** La fonction porte elle-même son drapeau « déjà liée » (voir `_bound`). */
type SwMessageListener = (() => void) & { _bound?: boolean };

function listenForSwMessages(): void {
  if (!navigator.serviceWorker || (listenForSwMessages as SwMessageListener)._bound) return;
  (listenForSwMessages as SwMessageListener)._bound = true;
