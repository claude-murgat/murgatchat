...
/** Sévérité d'une entrée du tampon. */
type LogLevel = "info" | "warn" | "error";

/** Une ligne du ring : horodatage, niveau, texte déjà tronqué. */
interface LogEntry {
  t: number;
  level: LogLevel;
  msg: string;
...
/** Contexte diagnostique poussé par l'appelant (jamais importé — voir en-tête). */
...
const ring: LogEntry[] = [];
let installed = false;
let context: LogContext = {}; // { userId, username, serverUrl, socket }
...
function push(level: LogLevel, text: unknown) {
  const s = String(text);
  ring.push({
    t: Date.now(),
    level,
...
export function logEvent(level: LogLevel, ...args: unknown[]) {
  push(level, fmt(args));
}
export const logInfo = (...a: unknown[]) => logEvent("info", ...a);
export const logWarn = (...a: unknown[]) => logEvent("warn", ...a);
export const logError = (...a: unknown[]) => logEvent("error", ...a);
...
/**
 * En-tête diagnostique. `appVersion` et `platform` sont toujours présents ; les
 * autres champs sont retirés quand ils sont vides (voir la purge plus bas), d'où
 * la signature d'index qui couvre aussi les clés passées par l'appelant.
 */
...
export function getDiagnostics(extra: Record<string, unknown> = {}): Diagnostics {
  const nav: Partial<Navigator> = typeof navigator !== "undefined" ? navigator : {};
  const scr: Partial<Screen> =
    typeof window !== "undefined" && window.screen ? window.screen : {};
...
export function dumpText(extra: Record<string, unknown> = {}) {
